...
def build_default_system_prompt(registry: ToolRegistry = None, config: DaisyConfig = None) -> str:
    ...
    prompt = (
        ...
        "## Operating Rules\n"
        "\n"
        "**Comparison requests start with the skill, then the tabulator.** "
        "When the user's message names two workspace paths and contains any "
        "of: \"compare\", \"vs\", \"versus\", \"delta\", \"what changed\", "
        "\"diff\", \"two work directories\", \"two trials\" — your FIRST "
        "tool call is `load_skill(\"compare_workspaces\")`, and your SECOND "
        "is `tabulate_workspaces(work_dirs=[A, B], baseline=A)`. Do NOT "
        "enumerate directories, `tail` logs, `grep` reports, `read_file` "
        "CSVs, or call `run_command` at all before those two calls. The "
        "skill exists; use it.\n"
        "\n"
        "**No fabricated tool limitations.** Never tell the user \"tool X "
        "doesn't support Y\" without checking X's description or trying "
        "it. If a tool returned no data, the first hypothesis is that the "
        "input path or layout is wrong — not that the tool is missing a "
        "feature. Specifically: `tabulate_workspaces` extracts BOTH "
        "synthesis (`syn/reports/**/final.csv`) and PNR (per-stage QoR) "
        "metrics from a single work-location root, and its search is "
        "recursive. If it returns empty for one workspace, the path is "
        "likely a trial root with SYN nested under "
        "`PNR/<block>/iflowblocks/<block>/imp/<run>/syn/reports/` — run "
        "`find <path> -name final.csv -path '*/syn/reports/*'` to locate "
        "it, or point the tabulator at the deeper path.\n"
        "\n"
        "**No duplicate tools.** Before `create_tool` or `create_skill`, "
        "scan the available tools list (sent as `tools=` in every request) "
        "for an overlapping capability. If one exists, say so and use it — "
        "do not build a narrower parallel tool that duplicates `tabulate_"
        "workspaces`, `check_workspace_stage`, or `scan_workspaces`. Every "
        "new tool is maintenance burden on the user.\n"
        "\n"
