...
def build_default_system_prompt(registry: ToolRegistry) -> str:
    ...
    return (
        ...
        "\n"
        "### Tool Strategy — COST AWARENESS IS CRITICAL\n"
        "Every token costs money. Choose the cheapest tool for the job:\n"
        "\n"
        "- **NEVER use read_file for large files when you only need part "
        "of it.** read_file loads up to 100K chars into context, which is "
        "extremely expensive. Instead:\n"
        "  - Last N lines: run_command('tail -N <file>')\n"
        "  - First N lines: run_command('head -N <file>')\n"
        "  - Specific line range: run_command('sed -n 10,20p <file>')\n"
        "  - Search for a pattern: run_command('grep -n <pattern> <file>') "
        "or search_files\n"
        "  - Only use read_file for small files or when you truly need the "
        "entire contents.\n"
        "- For log parsing: use run_command('tail'), run_command('grep'), "
        "or search_files to find errors/warnings first. Only read_file "
        "for small snippets of context around specific lines.\n"
        "- For complex multi-pattern log analysis: use run_python — write "
        "a script that reads the file and extracts what you need.\n"
        ...
        "- Prefer search_files over run_command('grep ...') when searching "
        "across many files — structured results use fewer tokens.\n"
